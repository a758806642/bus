...
$.ajax({
    type: 'POST',
    url: '/Common/GetAllCarSeat',
    async: false,
    cache: true,
    success: function (data) {
        for (var i = 0; i < data.length; i++) {
            carSeat += '<option value="' + data[i].ID + '">' + data[i].CarSeatName + '</option>';
        }
    },
    error: function () {
        //layer.msg("异常！");
...
// 	        '<input type="hidden" name="path{0}id" class="id" />' +
// 	        // ' <input type="text"  onfocus="this.blur();" readonly="readonly"  name="via" class="txt city" id="travel-start" />' +
// 	        ' <input type="text"  onfocus="this.blur();" readonly="readonly"  name="via[]" class="txt city" id="travel-start" />' +
...
// 	        '<dd>' +
// 	        '<div class="part">' +
// 	        '<input type="text" name="via_address[]" class="txt place input" placeholder="请输入附近的小区/街道/建筑物/商圈" style="width:360px;" autocomplete="off"/><input type="button" class="del" value="移除" />' +
...
// 	});
	...
	//航班号
	$("input[name='flightno']").keyup(function(e) {
		var URL = '__URL__';
		var geturl= URL+"/airline/flightno/" + $(this).val();
		$.ajax({
			url: geturl,
			cache:false,
			dataType:"json",
			...
			success: function(data) {
				if(data){
					$(".drome").val(data.arrivedrome);
					$("#message-flight-time").html(data.company+' '+data.arrivetime+' '+data.address+' '+data.arrivedrome);
					$("#flight-city").val(data.address);
					$(".ft2").val(data.arrivedrome);
					//alert($(".drome").val());
				} else {
					$("#message-flight-time").html("航班号不存在, 请输入正确的航班号");
				}
    ...
    $.checkvalidate = function() {
        ...
        //验证租车类型
        var checkradio = $("#travel").find("input:radio[name='car_type']:checked").val();
        var checkradio = typeof($("input[name='car_type']:checked").val());
        //alert(checkradio);
